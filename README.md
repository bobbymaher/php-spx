...
| _SPX_DROP_PROFILES_UNDER_MS_ | `0` | When set to a positive value, profiles whose total wall time is below this many milliseconds are discarded (no `.json` or compressed report file is written). Useful for keeping disk usage down when HTTP profiling is enabled broadly. The default `0` keeps every profile. Only applies to the `full` report type. |
